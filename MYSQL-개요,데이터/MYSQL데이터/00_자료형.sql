...
실수자료형
FLOAT  : 실수형 자료형 (DOUBLE보다 범위가 상대적으로 작음)
DOUBLE : 실수형 자료형 
...
VARCHAR : 변수 공간의 길이를 변경가능한 문자열
          실제 데이터의 길이만큼 저장
                                                          최대 255자(숫자,영어 기준) 저장 
TEXT    : 긴글자를 저장할 때 사용                           최대65,535 자 까지 저장 가능  
...
ENUM : 미리 정의된 값을 선택하는데 사용
        HTML에서 SELECT - OPTION CHECKBOX 처럼 지정된 범위에서만 선택 가능
        EX) ENUM('small','medium','large') = 작음 중간 큼 중 하나를 선택해서 db에서 
SET  : 여러 값을 가질 수 있는 집합을 정의하는 자료형 (다수 선택 가능)
        CHECKBOX 처럼 다수 선택해서 DB에 저장 
        EX) SET('독서','낚시','쇼핑') = 독서 낚시 쇼핑을 한 번에 저장 가능 

CREATE TABLE 테이블명칭(
    USER_ID INT PRIMARY KEY AUTO_INCRMENT,
