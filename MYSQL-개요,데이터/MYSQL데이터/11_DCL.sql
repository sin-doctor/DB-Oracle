...
관리자 계정 : 데이터베이스의 생성과 관리를 담당하는 계정
            모든 권한과 책임을 가지는 계정 
            ex) sys(최고관라자), system(sys에서 권한 몇개 제외된 관리자)
...
2) 객체 권한 : 특정 객체를 조작할 수 잇는 권한 
    ...
    */
    ...
    CREATE USER '유저아이디'@'LOCALHOST' BY '유저비밀번호';
    /* 데이터베이스 생성 */
    ...
    --> 소유자 계정은 KHCAFE 데이터베이스에서 MENT,ORDER,STORE 등 모든 테이블 접근 가능 
    ...
    */
    ...
    -- JUNIOR 가 이상한 행동을 하는 것 같음. 기존에 부여한 권한 중 INSERT UPDATE를 히수 
     REVOKE INSERT, UPDATE ON KHUSER.user TO 'JUNIOR'@'%';

    -- JUNIOR 가 이상한 행동을 보이는게 맞음! 혹시 모르니 모든 권한 회수
