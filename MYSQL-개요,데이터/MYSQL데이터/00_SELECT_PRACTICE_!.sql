/*
USE DATABASE; 명칭은
한 회사에서 여러 서비스를 제공하고 각 서비스에 따른 데이터베이스에 접속할 때 사용
...
CREATE TABLE USERPROFILE (
    ...
    username VARCHAR(50) NOT NULL,  		-- 유저 이름 (빈칸 허용 불가)
    email VARCHAR(100) NOT NULL,            -- 유저 이메일 (빈칸 허용 불가)
    BIRTHDATE DATE,							-- 유저 생년월일(날짜)
    ACCOUNT_BALANCE DECIMAL(10,2),  		-- 계좌 잔액 (정수 10자리, 소수자리 2자리) 			
    GENDER ENUM('MALE','FEMALE'),    		-- 유저 성별 (남성, 여성 중 택1)
    HOBBIES SET('READING','SPORTS','RUNNING') -- 유저 취미 (설정된 값 중 다중 선택)
);
...
VALUES
    ...
    -- 이광수 삭제 PRIMARY KEY 가 걸려잇으면 프라이머리 키 까지 모두 작성
