...
--	 	생성하다 	시퀁스를 	시퀀스이름설정 몇 번부터 시작할 것인가 증가 숫자 
...
-- 			가져가는 사람 수를 700번 부터 701, 702, ... 순차적으로 먹고간 인원 체크 
...
-- 트리거를 이용해서 테이블에 새로운 숫자값이 삽입될 때 시퀀스에 저장된 번호를 호출해서 ID 값을 자동 설정 
...
BEGIN 
	...
	:NEW.studentid = seq.user.NEXTVAL;
END;
...
SELECT * FROM student;
-- SELECT 문 활용해서 저장이 잘되었는지 확인 

CREATE OR REPLACE TRIGGER trg_user
BEFORE INSERT ON student 
FOR EACH ROW 
BEGIN 
    -- 시퀀스를 사용해서 student 테이블의 studentid 자동 생성
...
INSERT INTO student (s_id,studentname, studentage) VALUES (seq_user.NEXTVAL,'홍길동', 11);
